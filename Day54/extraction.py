...
def parse_email_text(email_text: str) -> dict:
    ...
    # Regex pattern for Name extraction (e.g., Name: Alice Smith or Member: Bob Jones)
    name_match = re.search(r'(?:Name|Member):\s*([A-Za-z\s]+)', email_text, re.IGNORECASE)
    if name_match:
        extracted_data["member_name"] = name_match.group(1).strip()

    # Regex pattern for Location extraction (e.g., Location: NY or State: CA)
    ...
    # Regex pattern for Date extraction (YYYY-MM-DD or MM/DD/YYYY)
